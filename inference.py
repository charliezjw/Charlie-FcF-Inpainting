...
if __name__ == "__main__":
    ...
    for i in range(1):
        in_imgs.append(
            Image.open(
                "datasets/1107/1699409929179.png"
    ...
    in_masks = []
    for i in range(1):
        t_in_mask = Image.open(
                    "datasets/1107/1699409929179_mask.png"
                    # "datasets/PG_full/airemove_f9b52a4e5e6787b27d73a0d11ba35b40_1696751112328_1696751114208_mask.png"
                    ).convert("RGB").convert("L")
        image_array = np.array(t_in_mask)
        image_array[image_array != 0] = 255
        final_image = Image.fromarray(image_array)
        in_masks.append(
            final_image
        )
    print(f"****{np.array(in_masks[0]).max()}")

    for i in range(20):
        start_time = time.perf_counter()
        out_img = generate_images(
            input_images=in_imgs, input_masks=in_masks, G=G, device=in_device
        )
        time_array.append(time.perf_counter() - start_time)
    print(time_array)
    # Image.fromarray(out_img[0]).save("datasets/1107/1699409929179_out.png")
